...
// Sticky pulse must be cleared so dataEpoch delegate rebuilds do not re-party.
assert.match(controller, /function clearCardCelebrationPulse\s*\(/)
assert.match(controller, /clearCelebrationPulse\.restart\(\)/)
assert.match(controller, /id:\s*clearCelebrationPulse/)
assert.match(controller, /celebrateGeneration\s*=\s*0/)
assert.match(controller, /celebrateProfileId\s*=\s*""/)
...
assert.match(card, /onCelebrateGenerationChanged:\s*tryPlayCelebration/)
